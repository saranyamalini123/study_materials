...
SELECT 
     ...
     SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS ww_gross_income_null_count,
...
-- output: 
-- Based on month we can see March month has 824 movies count than JAN,SEP,OCT. Least movie in the month DEC 438
...
-- select country from movie; (USA,India)
SELECT 
...
-- output:
-- 13 unique genres are there in genre 
...
-- output:
-- 'Drama' was the genre with 1078 movies for the year 2019.
...
-- output:
-- 3289 movies with unique genre are present.
...
-- output:
-- 'Thriller' has rank 3 with movie count 1484
...
-- output :
-- top 3 movies were kirket, love in kilnerry and gini heldia kathe
...
-- output:
-- median_rating with 7 of movie_count 2257
...
select * from prod_company_summary
WHERE prod_company_rank=1;

-- output:
-- 'Dream Warrior Pictures' & 'National Theatre Live' are with the rank 1 and movie_count 3.
...
-- output :
-- Drama with 24 movie count genre was highest in USA in march 2017
-- Top 3 were drama,comedy & action genre 
...
-- output:
-- 'The Brighton Miracle' was with highest rating 9.5 of genre drama

-- You should also try your hand at median rating and check whether the ‘median rating’ column gives any significant insights.
-- Q16. Of the movies released between 1 April 2018 and 1 April 2019, how many were given a median rating of 8?
-- Type your code below:
-- median rating =8
-- movies between 1 apr 2018 and 1 apr 2019
-- select * from movie where date_published between '2018-04-01' and '2019-04-01';
...
-- output:
-- 361 movies with 8 median rating between 1 apr 2018 & 1 apr 2019
...
-- output:
-- Germany has the highest votes with 106710 while italy with 77965 votes . 
...
-- first top 3 genres of movies with avg_Rating >8
-- top 3 directors with top 3 genres with avg_rating >8 
...
top_three_director AS (
	...
	WHERE r.avg_rating >8 
	AND g.genre IN ( SELECT genre FROM top_three_genre) 
	GROUP by director_name
	ORDER by movie_count DESC
)
SELECT * FROM top_three_director 
WHERE director_rank<=3;

-- output: 
-- James Mangold with 4 movie count ranks 1 & Anthony Russo,Joe Russo with 3 movie count ranks 2,3
...
-- output:
-- Mammootty & Mohanlal are the top actors with movie count 8 & 5.  
...
-- output:
-- Marvel Studios(2656967),Twentieth Century Fox(2411163) & Warner Bros.(2396057) are the top 3 production companies. 
...
SELECT 	n.name AS actor_name, 
		...
		DENSE_RANK() OVER(ORDER BY ROUND(SUM(r.avg_rating * r.total_votes)/ SUM(total_votes),2) DESC) AS actor_rank
	FROM names n
...
-- output:
-- Vijay Sethupathi is the top actor with avg_rating 8.42 and total_votes 23114 of movie_count 5.
-- followed by Fahadh Faasil and Yogi Babu.
...
SELECT 	n.name AS actress_name, 
		...
		DENSE_RANK() OVER(ORDER BY ROUND(SUM(r.avg_rating * r.total_votes)/ SUM(total_votes),2) DESC) AS actress_rank
	FROM names n
		INNER JOIN role_mapping rm  
			ON n.id=rm.name_id
		INNER JOIN ratings r 
...
-- output:
-- Taapsee Pannu is the top actress with avg_rating 7.74 followed by kritisanon & divya dutta . 
...
-- output:
-- The Fate of the Furious has the highest gross income under thriller genre of year 2017
-- The villain has the highest gross income under thriller genre of year 2018
-- Avengers:endgame has the highest gross income under Drama genre of year 2019
...
-- output:
-- Star Cinema & Twentieth Century Fox are the top production_company 
